...
function throwForBadAction(action) {
    var validActions = ['start', 'package', 'build', 'emulate'],
...
function executeStart(options) {
    ...
    self.template = options.template || 'starter';
    ...
    function addProperties() {
        ...
        shell.exec('echo "minimumClientVersion="' + self.version + ' >> app.properties');
        shell.exec('echo "releaseNumber="' + self.version + ' >> app.properties');
...
function executePackage(options) {
    ...
    // create mobile.zip
    shell.exec('zip -r  ' + mobileZipFile + ' ' + webSrcFolder);
    ...
    shell.exec('zip -r ' + packageDir + '/' + packageFile + ' ' + mobileZipFile + ' ' + propertiesFile + ' ' + iconFile);
